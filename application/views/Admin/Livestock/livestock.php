	...
	<div class="container-fluid">
		...
		<div class="card">
			<div class="card-header">
				<i class="fi fi-rr-list"></i>List of Livestock
			</div>
			<div class="card-body">
				<div class="d-flex justify-content-between mb-3">
					<a data-bs-toggle="modal" data-bs-target="#addLivestockModal" class="btn btn-primary">
						<i class="fi fi-rs-add"></i> Add New Livestock
					</a>
					<div class="btn-group">
					...
					<table id="basic-2" class="table table-striped table-bordered">
						...
						<tbody>
							...
							<?php foreach ($livestocks as $livestock): ?>
								<pre><?php print_r($livestock); ?></pre>
								<tr>
									<td><?= $count++ ?></td>
									<td><?= $livestock->ls_name ?></td>
									<td><?= $livestock->total_purchased ?></td>
									<td><?= $livestock->variant_count ?></td>
									<td>
										<button type="button" class="btn btn-warning btn-sm mb-2" data-bs-toggle="modal" data-bs-target="#editLivestockModal<?= $livestock->encrypted_id ?>">
											<i class="fi fi-rr-edit"></i> Edit
										</button>
										<a href="<?= base_url('admin/delete_livestock/' . $livestock->encrypted_id) ?>"
											<?= $livestock->variant_count > 0 ? 'onclick="return false;"' : '' ?>>
											<button type="button" class="btn btn-danger btn-sm mb-2"
												<?= $livestock->variant_count > 0 ? 'disabled' : '' ?>>
												<i class="fi fi-rr-trash"></i> Delete
											</button>
										</a>
										<button type="button" class="btn btn-dark btn-sm mb-2" data-bs-toggle="modal" data-bs-target="#addVariantModal<?= $livestock->encrypted_id ?>">
											<i class="fi fi-rr-add"></i> Add Variant
										</button>
										<a href="<?= base_url('admin/livestock_variants/' . $livestock->encrypted_id) ?>">
											<button type="button" class="btn btn-info btn-sm mb-2">
												<i class="fi fi-rr-eye"></i> View Variant
											</button>
										</a>
									</td>
								</tr>
								...
								<!-- Edit Modal -->
								<div class="modal fade" id="editLivestockModal<?= $livestock->encrypted_id ?>" tabindex="-1" aria-labelledby="editLivestockModalLabel" aria-hidden="true">
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
												<h4 class="modal-title" id="editLivestockModalLabel">Edit Livestock</h4>
												<button class="btn-close py-0" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
											</div>
											<div class="modal-body">
												<form role="form" action="<?= base_url('admin/update_livestock/' . $livestock->encrypted_id) ?>" method="post" enctype="multipart/form-data">
													<div class="form-group mb-3">
														<label for="livestockName">Name <span class="text-danger">*</span></label>
														<input type="text" class="form-control" name="name" id="livestockName" value="<?= $livestock->ls_name ?>" placeholder="Enter name" required>
													</div>
													<div class="form-group mb-3">
														<label for="livestockDescription">Description</label>
														<textarea name="description" class="form-control" id="livestockDescription" rows="3" placeholder="Enter description"><?= $livestock->ls_description ?></textarea>
													</div>
								...
								<!-- Variant Modal  -->
								<div class="modal fade" id="addVariantModal<?= $livestock->encrypted_id ?>" tabindex="-1" aria-labelledby="addVariantModalLabel" aria-hidden="true">
									<div class="modal-dialog">
										<div class="modal-content">
											<div class="modal-header">
